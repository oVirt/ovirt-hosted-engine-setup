...
class Plugin(plugin.PluginBase):
    ...
    def _verify_host_prefix_in_ip(self, ip):
        for hostip in self._getMyIPAddrList():
            if hostip.version == 6:
                v6_prefix = hostip.prefixlen
            else:
                v4_prefix = hostip.prefixlen
        if '/' not in ip:
            return '{a}/{pl}'.format(
                a=ip,
                pl=v6_prefix
    ...
    def _msg_validate_ip_cidr_subnet(self, proposed_cidr, v_ip, type):
        ...
        for ip in proposed_cidr:
            ip = self._verify_host_prefix_in_ip(ip)
            if netaddr.IPAddress(v_ip) in netaddr.IPNetwork(ip):
                return None
        return _(
        ...
        while not valid:
            ...
            if static:
                if interactive:
                    ...
                    # to fix here.
                    proposed_cidr = self._verify_host_prefix_in_ip(
                        proposed_cidr
                    )
    ...
    def _verify_fqdn_equals_to_host_subnet(self, fqdn):
        resolved_ips = self._hostname_helper.getResolvedAddresses(
            fqdn,
            self.environment[ohostedcons.NetworkEnv.FORCE_IPV6],
            self.environment[ohostedcons.NetworkEnv.FORCE_IPV4],
        )
        self.logger.info(
            _('The Engine VM FQDN was resolved into: {ip}.').format(
                ip=str(resolved_ips)[1:-1]
            )
        )
        host_ip = self._getMyIPAddrList()[0]
        return _(
            self._msg_validate_ip_cidr_subnet(resolved_ips, host_ip, "h")
        )
            ...
            if not self.environment[
                ...
                if self.environment[
                    ...
                    instancehname = self._hostname_helper.getHostname(
                        ...
                        extra_tests={
                            'test': self._verify_fqdn_equals_to_host_subnet,
                            'is_error': False,
                        },
                    ...
                    if (
                        ...
                        self.environment[
                            ...
                            note=_(
                                'Please provide the domain name you would '
                                'like to use for the engine appliance.\n'
